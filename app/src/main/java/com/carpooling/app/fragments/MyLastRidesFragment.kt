...
class MyLastRidesFragment : Fragment() {
    ...
    private fun showReviewDialog(booking: Booking) {
        ...
        dialogBinding.btnSubmit.setOnClickListener {
            ...
            if (rating < 1) {
                showErrorAlert("No Rating", "Please select a rating before submitting your review")
    ...
    private fun submitReview(booking: Booking, rating: Int, comment: String) {
        ...
        lifecycleScope.launch {
            try {
                // If there's a comment, check it for inappropriate content
                ...
                // Content is appropriate, proceed with review submission
                ...
                if (response.isSuccessful) {
                    showSuccessAlert("Review Submitted", getString(R.string.success_review))
                    reviewedRideIds.add(booking.rideId)
                } else {
                    showErrorAlert("Submission Failed", "Failed to submit review: ${response.message()}")
                }
            } catch (e: Exception) {
                showErrorAlert("Error", "Network error: ${e.message}")
            }
        }
    }
    ...
    private fun submitReport(booking: Booking, reason: String, description: String) {
        ...
        if (driverId == null) {
            showErrorAlert("Error", getString(R.string.error_driver_info_unavailable))
            return
        }
        
        if (description.isEmpty()) {
            showErrorAlert("Empty Description", "Please enter a description for your report")
            return
        }
        ...
        lifecycleScope.launch {
            try {
                ...
                // Content is appropriate, proceed with report submission
                ...
                if (response.isSuccessful) {
                    showSuccessAlert("Success", getString(R.string.report_submitted))
                } else {
                    showErrorAlert("Error", getString(R.string.error_submit_report))
                }
            } catch (e: Exception) {
                showErrorAlert("Error", "An error occurred: ${e.message}")
            }
        }
    }
